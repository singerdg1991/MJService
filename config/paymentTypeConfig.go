package config

import (
	"github.com/hoitek/Maja-Service/internal/paymenttype/config"
)

// PaymentTypeConfig is a global variable for the paymentType domain config
var PaymentTypeConfig config.ConfigType

// LoadPaymentTypeConfig loads the paymentType domain config
func LoadPaymentTypeConfig() config.ConfigType {
	PaymentTypeConfig = config.ConfigType{
		Debug:            AppConfig.Debug,
		Environment:      AppConfig.Environment,
		ApiPrefix:        AppConfig.ApiPrefix,
		ApiVersion1:      AppConfig.ApiVersion1,
		ApiVersion2:      AppConfig.ApiVersion2,
		DatabasePort:     AppConfig.DatabasePort,
		DatabaseName:     AppConfig.DatabaseName,
		DatabaseHost:     AppConfig.DatabaseHost,
		DatabaseUser:     AppConfig.DatabaseUser,
		DatabasePassword: AppConfig.DatabasePassword,
		DatabaseSslMode:  AppConfig.DatabaseSslMode,
		DatabaseTimeZone: AppConfig.DatabaseTimeZone,
	}
	return PaymentTypeConfig
}
