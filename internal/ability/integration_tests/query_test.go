...
import (
	...
	aConfig "github.com/hoitek/Maja-Service/internal/ability/config"
...
func TestAbilityQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadAbilityConfig()
	aConfig.AbilityConfig = &cfg

	// Create a new instance of ability service
	abilityService := service.NewAbilityService(repositories.NewAbilityRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewAbilityHandler(router.Init(), abilityService)
	if err != nil {
