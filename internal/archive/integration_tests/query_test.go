...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/archive/config"
...
func TestArchiveQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadArchiveConfig()
	cConfig.ArchiveConfig = &cfg

	// Create a new instance of archive service
	archiveService := service.NewArchiveService(repositories.NewArchiveRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewArchiveHandler(router.Init(), &archiveService, nil, nil)
	if err != nil {
