...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/city/config"
...
func TestCityQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadCityConfig()
	cConfig.CityConfig = &cfg

	// Create a new instance of city service
	cityService := service.NewCityService(repositories.NewCityRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewCityHandler(router.Init(), cityService)
	if err != nil {
