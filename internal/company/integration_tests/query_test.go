...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/company/config"
...
func TestCompanyQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadCompanyConfig()
	cConfig.CompanyConfig = &cfg

	// Create a new instance of company service
	companyService := service.NewCompanyService(repositories.NewCompanyRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewCompanyHandler(router.Init(), companyService)
	if err != nil {
