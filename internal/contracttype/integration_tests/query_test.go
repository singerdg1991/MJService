...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/contracttype/config"
...
func TestContractTypeQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadContractTypeConfig()
	cConfig.ContractTypeConfig = &cfg

	// Create a new instances
	contracttypeService := service.NewContractTypeService(repositories.NewContractTypeRepositoryStub(), nil)
	...
	handler, err := handlers.NewContractTypeHandler(router.Init(), &contracttypeService, userService)
	if err != nil {
