...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/diagnose/config"
...
func TestDiagnoseQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadDiagnoseConfig()
	cConfig.DiagnoseConfig = &cfg

	// Create a new instance of diagnose service
	diagnoseService := service.NewDiagnoseService(repositories.NewDiagnoseRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewDiagnoseHandler(router.Init(), &diagnoseService)
	if err != nil {
