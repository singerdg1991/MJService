...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/email/config"
...
func TestEmailQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadEmailConfig()
	cConfig.EmailConfig = &cfg

	// Create a new instance of email service
	emailService := service.NewEmailService(repositories.NewEmailRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewEmailHandler(router.Init(), &emailService, nil)
	if err != nil {
