...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/equipment/config"
...
func TestEquipmentQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadEquipmentConfig()
	cConfig.EquipmentConfig = &cfg

	// Create a new instances
	equipmentService := service.NewEquipmentService(repositories.NewEquipmentRepositoryStub(), nil)
	...
	handler, err := handlers.NewEquipmentHandler(router.Init(), &equipmentService, userService)
	if err != nil {
