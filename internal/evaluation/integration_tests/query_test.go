...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/evaluation/config"
...
func TestEvaluationQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadEvaluationConfig()
	cConfig.EvaluationConfig = &cfg

	// Create a new instances
	evaluationService := service.NewEvaluationService(repositories.NewEvaluationRepositoryStub(), nil)
	...
	handler, err := handlers.NewEvaluationHandler(router.Init(), &evaluationService, nil, userService)
	if err != nil {
