...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/geartype/config"
...
func TestGearTypeQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadGearTypeConfig()
	cConfig.GearTypeConfig = &cfg

	// Create a new instance of geartype service
	geartypeService := service.NewGearTypeService(repositories.NewGearTypeRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewGearTypeHandler(router.Init(), geartypeService)
	if err != nil {
