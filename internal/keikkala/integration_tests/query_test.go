...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/keikkala/config"
...
func TestKeikkalaQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadKeikkalaConfig()
	cConfig.KeikkalaConfig = &cfg

	// Create a new instance of keikkala service
	keikkalaService := service.NewKeikkalaService(repositories.NewKeikkalaRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewKeikkalaHandler(router.Init(), &keikkalaService, nil)
	if err != nil {
