...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/license/config"
...
func TestLicenseQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadLicenseConfig()
	cConfig.LicenseConfig = &cfg

	// Create a new instances
	licenseService := service.NewLicenseService(repositories.NewLicenseRepositoryStub(), nil)
	...
	handler, err := handlers.NewLicenseHandler(router.Init(), &licenseService, userService)
	if err != nil {
