...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/limitation/config"
...
func TestLimitationQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadLimitationConfig()
	cConfig.LimitationConfig = &cfg

	// Create a new instances
	limitationService := service.NewLimitationService(repositories.NewLimitationRepositoryStub(), nil)
	userService := uService.NewUserService(uRepositories.NewUserRepositoryStub(), nil, nil)

	// Create a new instances
	handler, err := handlers.NewLimitationHandler(router.Init(), &limitationService, userService)
