...
import (
	...
	"github.com/hoitek/Maja-Service/internal/limitation/handlers"
	"github.com/hoitek/Maja-Service/internal/limitation/ports"
	"github.com/hoitek/Maja-Service/internal/limitation/repositories"
	"github.com/hoitek/Maja-Service/internal/limitation/service"
...
// module is a module for the limitation domain
type module struct {
...
// Module is a global variable for the limitation domain module
var Module = &module{}

// GetService returns a new instance of the limitation service
func (m *module) GetService(pDB *sql.DB) ports.LimitationService {
	// limitation repository database based on the environment
	limitationRepositoryPostgresDB := exp.TerIf[ports.LimitationRepositoryPostgresDB](
		m.Config.Environment == constants.ENVIRONMENT_TESTING,
		repositories.NewLimitationRepositoryStub(),
		repositories.NewLimitationRepositoryPostgresDB(pDB),
	)
	// limitation service and inject the limitation repository database and grpc
	limitationService := service.NewLimitationService(limitationRepositoryPostgresDB, m.MinIOStorage)
...
// RegisterHTTP registers the limitation domain http routes
func (m *module) RegisterHTTP(r *mux.Router) (handlers.LimitationHandler, error) {
	// Check if router is nil
	if r == nil {
		return handlers.LimitationHandler{}, errors.New("router can not be nil")
	}

	// Create a new group router to scope any middleware here and prevent polluting the global router
	r = r.PathPrefix("/").Subrouter()

	// Use Json middleware for limitation module
	r.Use(json.Middleware)

	// Create a new limitation handler
	handler, err := handlers.NewLimitationHandler(
		r,
		m.GetService(m.PostgresDB),
		m.GetUserService(m.PostgresDB, m.MongoDB),
	)
	if err != nil {
		return handlers.LimitationHandler{}, err
	}

	// Return the limitation handler
	return handler, nil
}
