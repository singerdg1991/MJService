...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/medicine/config"
...
func TestMedicineQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadMedicineConfig()
	cConfig.MedicineConfig = &cfg

	// Create a new instance of medicine service
	medicineService := service.NewMedicineService(repositories.NewMedicineRepositoryStub(), nil)
	...
	handler, err := handlers.NewMedicineHandler(router.Init(), &medicineService, userService)
	if err != nil {
