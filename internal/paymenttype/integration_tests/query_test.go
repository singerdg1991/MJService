...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/paymenttype/config"
...
func TestPaymentTypeQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadPaymentTypeConfig()
	cConfig.PaymentTypeConfig = &cfg

	// Create a new instance of paymenttype service
	paymenttypeService := service.NewPaymentTypeService(repositories.NewPaymentTypeRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewPaymentTypeHandler(router.Init(), paymenttypeService)
	if err != nil {
