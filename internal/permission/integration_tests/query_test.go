...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/permission/config"
...
func TestPermissionQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadPermissionConfig()
	cConfig.PermissionConfig = &cfg

	// Create a new instances
	permissionService := service.NewPermissionService(repositories.NewPermissionRepositoryStub(), nil)
	...
	handler, err := handlers.NewPermissionHandler(router.Init(), &permissionService, userService)
	if err != nil {
