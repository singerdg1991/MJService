...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/prescription/config"
...
func TestPrescriptionQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadPrescriptionConfig()
	cConfig.PrescriptionConfig = &cfg

	// Create a new instance of prescription service
	prescriptionService := service.NewPrescriptionService(repositories.NewPrescriptionRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewPrescriptionHandler(router.Init(), &prescriptionService, nil)
	if err != nil {
