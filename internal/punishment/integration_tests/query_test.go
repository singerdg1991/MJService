...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/punishment/config"
...
func TestPunishmentQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadPunishmentConfig()
	cConfig.PunishmentConfig = &cfg

	// Create a new instances
	punishmentService := service.NewPunishmentService(repositories.NewPunishmentRepositoryStub(), nil)
	...
	handler, err := handlers.NewPunishmentHandler(router.Init(), &punishmentService, userService)
	if err != nil {
