...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/reward/config"
...
func TestRewardQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadRewardConfig()
	cConfig.RewardConfig = &cfg

	// Create a new instances
	rewardService := service.NewRewardService(repositories.NewRewardRepositoryStub(), nil)
	...
	handler, err := handlers.NewRewardHandler(router.Init(), &rewardService, userService)
	if err != nil {
