...
import (
	...
	"github.com/hoitek/Maja-Service/internal/reward/handlers"
	"github.com/hoitek/Maja-Service/internal/reward/ports"
	"github.com/hoitek/Maja-Service/internal/reward/repositories"
	"github.com/hoitek/Maja-Service/internal/reward/service"
...
// module is a module for the reward domain
type module struct {
...
// Module is a global variable for the reward domain module
var Module = &module{}

// GetService returns a new instance of the reward service
func (m *module) GetService(pDB *sql.DB) ports.RewardService {
	// reward repository database based on the environment
	rewardRepositoryPostgresDB := exp.TerIf[ports.RewardRepositoryPostgresDB](
		m.Config.Environment == constants.ENVIRONMENT_TESTING,
		repositories.NewRewardRepositoryStub(),
		repositories.NewRewardRepositoryPostgresDB(pDB),
	)
	// reward service and inject the reward repository database and grpc
	rewardService := service.NewRewardService(rewardRepositoryPostgresDB, m.MinIOStorage)
...
// RegisterHTTP registers the reward domain http routes
func (m *module) RegisterHTTP(r *mux.Router) (handlers.RewardHandler, error) {
	// Check if router is nil
	if r == nil {
		return handlers.RewardHandler{}, errors.New("router can not be nil")
	}

	// Create a new group router to scope any middleware here and prevent polluting the global router
	r = r.PathPrefix("/").Subrouter()

	// Use Json middleware for reward module
	r.Use(json.Middleware)

	// Create a new reward handler
	handler, err := handlers.NewRewardHandler(
		r,
		m.GetService(m.PostgresDB),
		m.GetUserService(m.PostgresDB, m.MongoDB),
	)
	if err != nil {
		return handlers.RewardHandler{}, err
	}
