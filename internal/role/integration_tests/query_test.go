...
func TestRoleQueryIntegration(t *testing.T) {
	//// Load default config
	//config.LoadDefault()
	//cfg := config.LoadRoleConfig()
	//cConfig.RoleConfig = &cfg
	//
	//// Create a new instance of role service
	//roleService := service.NewRoleService(repositories.NewRoleRepositoryStub())
	//
	//// Create a new instance of HTTP server handler
	//handler, err := handlers.NewRoleHandler(router.Init(), roleService)
	//if err != nil {
