...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/service/config"
...
func TestServiceQueryIntegration(t *testing.T) {
	...
	cConfig.ServiceConfig = &cfg

	// Create a new instance of service service
	serviceService := service.NewServiceService(repositories.NewServiceRepositoryStub(), nil)
	...
	handler, err := handlers.NewServiceHandler(router.Init(), &serviceService, userService)
	if err != nil {
