...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/servicegrade/config"
...
func TestServiceGradeQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadServiceGradeConfig()
	cConfig.ServiceGradeConfig = &cfg

	// Create a new instances
	servicegradeService := service.NewServiceGradeService(repositories.NewServiceGradeRepositoryStub(), nil)
	...
	handler, err := handlers.NewServiceGradeHandler(router.Init(), &servicegradeService, userService)
	if err != nil {
