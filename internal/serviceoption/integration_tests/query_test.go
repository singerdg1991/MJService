...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/serviceoption/config"
...
func TestServiceQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadServiceOptionConfig()
	cConfig.ServiceOptionConfig = &cfg

	// Create a new instances
	serviceTypeService := service.NewServiceService(repositories.NewServiceOptionRepositoryStub(), nil)
	...
	handler, err := handlers.NewServiceOptionHandler(router.Init(), &serviceTypeService, userService)
	if err != nil {
