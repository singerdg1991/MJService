...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/servicetype/config"
...
func TestServiceTypeQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadServiceTypeConfig()
	cConfig.ServiceTypeConfig = &cfg

	// Create a new instances
	serviceTypeService := service.NewServiceTypeService(repositories.NewServiceTypeRepositoryStub(), nil)
	...
	handler, err := handlers.NewServiceTypeHandler(router.Init(), &serviceTypeService, userService)
	if err != nil {
