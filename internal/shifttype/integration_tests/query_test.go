...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/shifttype/config"
...
func TestShiftTypeQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadShiftTypeConfig()
	cConfig.ShiftTypeConfig = &cfg

	// Create a new instance of shifttype service
	shifttypeService := service.NewShiftTypeService(repositories.NewShiftTypeRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewShiftTypeHandler(router.Init(), shifttypeService)
	if err != nil {
