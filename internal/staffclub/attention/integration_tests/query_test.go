...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/staffclub/attention/config"
...
func TestAttentionQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadAttentionConfig()
	cConfig.AttentionConfig = &cfg

	// Create a new instance of attention service
	attentionService := service.NewAttentionService(repositories.NewAttentionRepositoryStub(), nil)
	...
	handler, err := handlers.NewAttentionHandler(router.Init(), &attentionService, nil, userService)
	if err != nil {
