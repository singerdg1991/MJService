...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/staffclub/grace/config"
...
func TestGraceQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadGraceConfig()
	cConfig.GraceConfig = &cfg

	// Create a new instances
	graceService := service.NewGraceService(repositories.NewGraceRepositoryStub(), nil)
	...
	handler, err := handlers.NewGraceHandler(router.Init(), &graceService, nil, userService)
	if err != nil {
