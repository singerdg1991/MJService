...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/staffclub/holiday/config"
...
func TestHolidayQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadHolidayConfig()
	cConfig.HolidayConfig = &cfg

	// Create a new instance of holiday service
	holidayService := service.NewHolidayService(repositories.NewHolidayRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewHolidayHandler(router.Init(), &holidayService, nil)
	if err != nil {
