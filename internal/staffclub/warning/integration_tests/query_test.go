...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/staffclub/warning/config"
...
func TestWarningQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadWarningConfig()
	cConfig.WarningConfig = &cfg

	// Create a new instances
	warningService := service.NewWarningService(repositories.NewWarningRepositoryStub(), nil)
	...
	handler, err := handlers.NewWarningHandler(router.Init(), &warningService, nil, userService)
	if err != nil {
