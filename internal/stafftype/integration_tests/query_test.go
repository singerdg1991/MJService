...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/stafftype/config"
...
func TestStaffTypeQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadStaffTypeConfig()
	cConfig.StaffTypeConfig = &cfg

	// Create a new instance of staffType service
	staffTypeService := service.NewStaffTypeService(repositories.NewStaffTypeRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewStaffTypeHandler(router.Init(), &staffTypeService)
	if err != nil {
