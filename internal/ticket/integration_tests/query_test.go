...
import (
	...
	cConfig "github.com/hoitek/Maja-Service/internal/ticket/config"
...
func TestTicketQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadTicketConfig()
	cConfig.TicketConfig = &cfg

	// Create a new instance of ticketCategory service
	ticketCategoryService := service.NewTicketService(repositories.NewTicketRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewTicketHandler(router.Init(), &ticketCategoryService, nil)
	if err != nil {
