...
import (
	...
	vtConfig "github.com/hoitek/Maja-Service/internal/vehicletype/config"
...
func TestVehicleTypeTypeQueryIntegration(t *testing.T) {
	// Load default config
	config.LoadDefault()
	cfg := config.LoadVehicleTypeConfig()
	vtConfig.VehicleTypeConfig = &cfg

	// Create a new instance of vehicletype service
	vehicletypeService := service.NewVehicleTypeService(repositories.NewVehicleTypeRepositoryStub(), nil)

	// Create a new instance of HTTP server handler
	handler, err := handlers.NewVehicleTypeHandler(router.Init(), vehicletypeService)
	if err != nil {
