...
import (
	...
	"github.com/hoitek/OpenEngine/constants"
...
func (p *openEngine) AddErrorResponses(errorResponses engine.ErrorResponses, errorResponseRefs ...string) OpenEngine {
	...
	for statusCode, response := range errorResponses {
		...
		if defaultRef != "" || response.Content.MultipartFormData.Schema.Ref != "" {
			response.Content.MultipartFormData.Schema.Ref = engine.TerIf(
				response.Content.MultipartFormData.Schema.Ref == "", defaultRef, "#/components/schemas/"+response.Content.MultipartFormData.Schema.Ref,
			)
		}
