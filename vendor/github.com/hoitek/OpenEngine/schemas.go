...
func (p *openEngine) mapSchemaFieldsToSchemaDict(list []*ast.Field, structName string, schemasDict *engine.SchemasDict) {
	for _, field := range list {
		...
		case *ast.Ident:
			...
			if tagValues.Format != "" {
				format = tagValues.Format
			} else {
				format = engine.TerIf(field.Type.(*ast.Ident).Obj == nil, engine.OpenAPIFormats(t), "object")
			}
			ref = engine.TerIf(tagValues.Ref != "", "#/components/schemas/"+tagValues.Ref, "")
		...
		case *ast.IndexExpr, *ast.SelectorExpr, *ast.ArrayType:
			if tagValues.Format == "binary" {
				fieldType := fmt.Sprintf("%T", field.Type)
				if fieldType == "*ast.ArrayType" {
					tp = "array"
					items = &engine.PropertyItems{
						Type:   "string",
						Format: "binary",
		...
		case *ast.StarExpr:
			// Get the type of the field
			x := field.Type.(*ast.StarExpr).X
			switch xValue := x.(type) {
			case *ast.Ident:
				t := xValue.Name
				tp = engine.TerIf(xValue.Obj == nil, engine.OpenAPITypes(t), "object")
				if tagValues.Format != "" {
					format = tagValues.Format
				} else {
					format = engine.TerIf(xValue.Obj == nil, engine.OpenAPIFormats(t), "object")
				}
				ref = engine.TerIf(tagValues.Ref != "", "#/components/schemas/"+tagValues.Ref, "")
				maxLength = engine.TerIf(xValue.Obj == nil, maxLength, 0)
				minLength = engine.TerIf(xValue.Obj == nil, minLength, 0)
				maximum = engine.TerIf(xValue.Obj == nil, maximum, 0)
				minimum = engine.TerIf(xValue.Obj == nil, minimum, 0)
				if tagValues.Format == "binary" {
					tp = "string"
					format = "binary"
				}
			case *ast.SelectorExpr:
				t := xValue.Sel.Name
				tp = engine.TerIf(xValue.Sel.Obj == nil, engine.OpenAPITypes(t), "object")
				if tagValues.Format != "" {
					format = tagValues.Format
				} else {
					format = engine.TerIf(xValue.Sel.Obj == nil, engine.OpenAPIFormats(t), "object")
				}
				ref = engine.TerIf(tagValues.Ref != "", "#/components/schemas/"+tagValues.Ref, "")
				maxLength = engine.TerIf(xValue.Sel.Obj == nil, maxLength, 0)
				minLength = engine.TerIf(xValue.Sel.Obj == nil, minLength, 0)
				maximum = engine.TerIf(xValue.Sel.Obj == nil, maximum, 0)
				minimum = engine.TerIf(xValue.Sel.Obj == nil, minimum, 0)
